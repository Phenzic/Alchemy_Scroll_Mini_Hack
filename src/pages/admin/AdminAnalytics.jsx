...
import {getAllProducts, getOrders} from "../../utils/firebase/index"
...
const AdminAnalytics = () => {
  ...
  const fetchProducts = async()=>{
    ...
    const recentProducts = products.filter((eachProduct)=>{
      const today = new Date();
      const date = new Date(eachProduct.updatedAt.seconds*1000);
      const yesterday = new Date(date.getTime() - 86400000).getDate();
      if(today.getDate()-yesterday<=5){
        return eachProduct;
      }
                    ...
                    {recentProducts.map((eachProduct, key) => {
                        return (
                          <tr
                            ...
                            key={key}
                          >
                            <td className="px-6 py-4">{eachProduct?.name || "N/A"}</td>
                            <td className="px-6 py-4">{eachProduct?.price || "N/A"}</td>
                            <td className="px-6 py-4">{eachProduct?.quantity || "N/A"}</td>
                            {/* <td className="px-6 py-4">{timeAgo(eachProduct.updatedAt?.seconds)}</td> */}
